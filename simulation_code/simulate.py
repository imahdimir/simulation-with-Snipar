...
def make_causal_out_and_save(p , ar) :
    ...
    if ar.v_indir == 0 :
        ...
        for i in range(len(p.haps)) :
            ...
            _o = np.vstack((p.snp_ids[i] , p.alleles[i][: , 0] ,
                            p.alleles[i][: , 1] , a_chr , a_chr_v1 ,
                            a_chr_v10)).T

            causal_out[snp_count :(snp_count + p.snp_ids[i].shape[0]) , :] = _o

            snp_count += p.snp_ids[i].shape[0]

        _cols = ['SNP' , 'A1' , 'A2' , 'direct' , 'direct_v1' , 'direct_v10']
        _cols = np.array(_cols).reshape((1 , 6))

        causal_out = np.vstack((_cols , causal_out))
        ...
        _cols = np.array(_cols).reshape((1 , 5))
        causal_out = np.vstack((_cols , causal_out))
